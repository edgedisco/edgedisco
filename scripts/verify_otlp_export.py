...
def main():
    ...
    expected = {}
    for index, (running, version) in enumerate(((True, None), (False, None), (True, "1.2.3"), (True, "2.0.0"))):
        db.ingest(device, {"scan_id": f"export-integration-{index}",
            "observed_at": (datetime.now(timezone.utc) - timedelta(seconds=10)).isoformat(),
            "assets": [{"fingerprint": "integration-runtime", "kind": "agent_runtime", "name": "CrewAI", "vendor": "CrewAI",
        ...
        with db.connect() as conn:
            row = conn.execute("SELECT * FROM otlp_outbox WHERE status='pending'").fetchone()
        ...
        expected[row['id']] = {"attributes": attrs, "timestamp": str(record.time_unix_nano), "observed_timestamp": str(record.observed_time_unix_nano)}
    ...
    summary = {"endpoint": args.endpoint, "device_id": device, "delivered": len(expected), "verified_in_loki": len(found),
               "checks": ["running", "stopped", "versions", "simulation", "host_relationship", "event_timestamp", "observed_timestamp", "gzip"],
